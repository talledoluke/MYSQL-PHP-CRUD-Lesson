...
if(isset($_POST["submitButton"])){
    ...
    $query = "INSERT INTO `ranulo-talledo`(`student_id`, `fname`, `mname`, `lname`, `birthday`, `email`) VALUES ('$student_id','$fname','$mname','$lname','$birthday', '$email')";
    $query_result = mysqli_query( $con, $query );

    if($query_result){
        $_SESSION['status'] = "nabutang na kol, aleluya!";
        $_SESSION['status_code'] = "success";
        header("Location: index.php");
        exit();
    }
...
if(isset($_POST["update"])){
    ...
    $query = "UPDATE `ranulo-talledo` SET `student_id`='$student_id',`fname`='$fname',`mname`='$mname',`lname`='$lname',`birthday`='$birthday',`email`='$email',`date_Created`='$date_Created' WHERE `id` ='$id'";
    
    $query_result = mysqli_query( $con, $query );

    if($query_result){
        $_SESSION['status'] = "Na update na kol, char!";
        $_SESSION['status_code'] = "success";
        header("Location: index.php");
        exit();
    }else{
...
if(isset($_POST["delete"])){
    ...
    $query = "DELETE FROM `ranulo-talledo` WHERE id = $id";
